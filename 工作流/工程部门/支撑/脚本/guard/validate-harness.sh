#!/bin/bash
# validate-harness.sh — 框架完整性自检（2026-08-04 重组后 SCRIPT_DIR 定位）
...
echo "--- 规则 ---"
check "$BASE/支撑/规则/00-core.md"
check "$BASE/支撑/规则/01-language.md"
check "$BASE/支撑/规则/02-git.md"
check "$BASE/支撑/规则/03-security.md"
check "$BASE/支撑/规则/04-quality.md"
check "$BASE/支撑/规则/05-architecture.md"
check "$BASE/流程/测试/reviewer-criteria.md"
...
check "$BASE/支撑/脚本/guard/validate-harness.sh"
check "$BASE/支撑/脚本/guard/task-counter.sh"

echo "--- 模板与运行文档 ---"
check "$BASE/支撑/模板/project-CLAUDE.md"
check "$BASE/支撑/模板/skill-SKILL.md"
check "$BASE/支撑/模板/project-init.sh"
check "$BASE/支撑/文档/audit/regression-log.md"
check "$BASE/支撑/文档/audit/last-optimization.txt"
check "$BASE/支撑/文档/changelog.md"
