...
## 2026-08-05：R-010 助理中心制（主对话=助理）

- **触发**：老板拍板（2026-08-05）——框架多余内容多，学习部/总理/规划/交易部整合进主对话，日常最低 token
- **变更**：
  - 根 CLAUDE.md 重写为**助理入口**（身份/分工/记忆分层/导航），日常主窗口最低 token
  - 组织架构单一来源更新（工程部门/支撑/制度/组织架构.md）：主对话=助理（业务+规划一体）；工程部/质检部保留子 agent；trader/learner/公共服务部经理保留为工具 agent；其余为资产库
  - 优化部门并入助理（台账/审计主对话执行，CLAUDE.md 改为工具资产说明）
  - 删除清单 D1-D10：总理/组织架构.md（旧版）、optimization/ 残留、重复 JSON、产出/临时、废弃回测树、旧 settings、过期方案、确认书双份、learn 技能副本合一（junction 指向技能库）
  - 归档：R-001~R-004 转交单 + 执行状态移档案室归档
  - 全工作区旧称清理：内审部/培训部/总裁办/工程中枢/公共服务部/规划部/双库→现行口径（历史档案保留标注）
  - 学习部协议/规范单库化；助理规则.md/00-core.md/角色文件/技能描述同步修正
  - 交易部门/CLAUDE.md 目录结构与路径对齐（工具链/脚本）
  - 记忆分层落地：auto-memory 架构记忆更新为助理中心制
...
- **变更**：
  ...
  - **资源调动优化**：四角色文件"自我提升（提案机制）"低频节提取为共享 `工程部门/支撑/制度/提案机制.md`（单一来源，角色文件合计 14.5KB→11KB 省 24%）；`工作流/CLAUDE.md` 新增「资源按需原则」纪律（不主动全量读/指针式引用/技能按需/先查清单）
