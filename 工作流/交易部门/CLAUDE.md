...
## 执行卡每日送审（R-074，2026-08-12 老板拍板）

- **流程**：每日执行卡生成（18:40 计划任务）→ **总助派交易部独立复核**（投资研究员人格，不带对话上下文，基于文件+数据独立判断）→ 复核意见 → **复核通过后才呈报老板**
- 复核重点：持仓确认/平仓判定、挂单有效性（R-072 四态）、补仓判定（现金口径）、资金占用、S 级全览
- 输出格式：【交易部复核意见 · 日期】确认项/问题项（P0 必须改/P1 建议改/P2 细节）+ 结论（通过/需返工）
- P0 问题必须修订后重新复核；P1 呈报老板时附注；P2 记录不阻塞
